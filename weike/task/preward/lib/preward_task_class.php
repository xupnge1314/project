...
class preward_task_class extends keke_task_class {
	...
	public function check_work_if_standard($type) {
		...
		if ($type == 'hand') {
			if ($this->_task_config ['work_percent'] == 0) {
				...
				$total_hand_count = intval($hand_count) + intval($hege_count);
				if ($total_hand_count < $totle_count) {
					return true;
				} else {
					return false;
				}
	...
	public function dispose_task_return($action) {
		if(in_array($this->_task_info['task_status'], array('7','8','9','10')) ){
			return false;
		}
		$uid = $this->_uid;
			...
			if (time () > intval ( $this->_task_info ['sub_time'] )) {
				...
				if (intval ( $this->_task_info ['work_num'] ) > 0) {
					if ($this->set_task_status ( 3 )) {
						...
						keke_msg_class::notify_user ( $this->_guid, $this->_gusername, 'timeout', '任务选稿', $arr,1 );
					}
				} else {
