			...
			case "stoptask":
				...
				if($arrworkInfo){
					...
					foreach ( $arrworkInfo as $k => $v ) {
					    ...
					    $prize_real_cash = $prize_cash * (1 - $config ['task_fail_rate'] / 100);
				...
				foreach($work_info as $v2){
					if($work_info){
						...
						db_factory::execute("update ".TABLEPRE."witkey_task_work set work_status=0 where task_id='".intval($task_id)."' and work_status in (1,2,3)");
					}
				}
