...
abstract class keke_auth_base_class {
	...
	public function del_auth($auth_ids,$url=null) {
		...
		if(is_array($auth_ids) && !empty($auth_ids)){
			$authLists=$this->get_auth_info(implode(",",$auth_ids));
		}elseif(intval($auth_ids) > 0){
			$authLists=$this->get_auth_info($auth_ids);
		}else{
			kekezu::admin_show_msg($this->auth_lang(). $_lang['apply_not_exist_delete_fail'],$url);
		}
		if(!empty($authLists) && is_array($authLists)){
			foreach ($authLists as $v){
				if($v['auth_status'] !='1'){
					$this->_tab_obj->del($this->_primary_key,$v[$this->_primary_key]);
					$this->del_auth_record($v['uid']);
					switch ($v['auth_code']) {
						case 'enterprise':
							$this->set_user_role($v['uid'],'not_pass');
						break;
						default:
							;
						break;
			...
			kekezu::admin_show_msg($this->auth_lang(). "申请删除失败",$url,3,'','warning');
		}
	}
