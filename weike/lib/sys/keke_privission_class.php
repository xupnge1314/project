...
abstract class keke_privission_class {
	...
	public static function get_operate_num($task_id, $uid, $op_code, $priv) {
		...
		$task_id and $sql = sprintf ( " select count(%s) from %s a left join %switkey_task b on %s=b.task_id where a.uid='%d' and b.model_id='%d' and %s>%d and b.task_id='%d'", $pk, $table, TABLEPRE, $join_pk, $uid, $model_id, $time_fields, time () - 24 * 3600, $task_id ) or $sql = sprintf ( " select count(%s) from %s a left join %switkey_task b on %s=b.task_id where a.uid='%d' and b.model_id='%d' and %s>%d", $pk, $table, TABLEPRE, $join_pk, $uid, $model_id, $time_fields, time () - 24 * 3600 );
