...
class CustomClass {
	...
	static function getExtDataList($objid,$model_id){
		...
		$result  =  db_factory::query("
				...
				FROM `".TABLEPRE."witkey_custom_fields_ext` AS a 
				LEFT JOIN  `".TABLEPRE."witkey_custom_fields` AS b 
				ON a.c_id = b.id 
	...
	static function delExtDataByObjId($objid,$model_id){
		if(!self::checkCustom($model_id)) return false;
		return db_factory::execute("delete FROM `".TABLEPRE."witkey_custom_fields_ext` WHERE objid = ".intval($objid)." and model_id = ".intval($model_id));
	}
}
?>