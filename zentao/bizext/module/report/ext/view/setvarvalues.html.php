...
<div class='col-md-3 col-sm-6' style='width:220px'>
  <div class='input-group'>
    <span class='input-group-addon'><?php echo isset($sqlVars['showName'][$i]) ? $sqlVars['showName'][$i] : $varName?></span>
    <?php
...
function setDateField(query)
{
    ...
    $(query).datetimepicker('remove').datetimepicker(dtOptions).on('show', function(e)
    {
        var $e = $(e.target);
        var ePos = $e.offset();
        $period.css({'left': ePos.left + 175, 'top': ePos.top + 29, 'min-height': $('.datetimepicker').outerHeight()}).show().attr('data-target', $e.attr('id')).find('li.active').removeClass('active');
