$(document).ready(function()
{
    ...
    $bugForm.submit(function()
    {
        $(this).ajaxSubmit(
        {
            success:function(json)
            {
                createBug($.parseJSON(json), null, null, 3);
                $diffCode.removeClass('with-action-row');
    ...
    parent$('.modal-header h4.modal-title span').html($('.revisions').html());
...
{
    link = createLink('repo', 'ajaxGetProjects', 'productID=' + $(select).children('option:selected').val());
    $(select).closest('.bugFormContainer').find('select[name=project]').parent().load(link, '', function(){$('#project').chosen();});
    moduleLink = createLink('tree', 'ajaxGetOptionMenu', 'productID=' + $(select).children('option:selected').val() + '&viewtype=bug&rootModuleID=0&returnType=html');
