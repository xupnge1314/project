...
if (isset ( $action )) {
	...
	$strUnionSql = ' SELECT uid, work_id id,task_id,work_time ontime,work_status as status,work_file as file FROM '.TABLEPRE.'witkey_task_work where uid='.$gUid.'
             union all
             SELECT uid, bid_id id,task_id,bid_time ontime,bid_status as status,hasdel as file FROM '.TABLEPRE.'witkey_task_bid where uid='.$gUid;
	...
	if(is_array($arrLists)){
	    foreach($arrLists as $k=>$v){
	        $arrTaskInfo = db_factory::get_one("select * from ".TABLEPRE."witkey_task where task_id=".intval($v['task_id']));
	        if($arrTaskInfo){
