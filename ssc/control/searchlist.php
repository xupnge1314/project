...
if($arrHwStatus[0]['v'] == 'open'){
	if($arrUpdateStatus[0]['v'] == 'auto'){
	    if($ky){
	        $searchlist=db_factory::query("select words from ".TABLEPRE."witkey_hotwords where words like '%$ky%' and auto=1 order by count desc limit 5");
	    }else{
	        $searchlist=db_factory::query("select words from ".TABLEPRE."witkey_hotwords where auto=1 order by count desc limit 5");
	    }
	    if($searchlist){
	    ...
	    if($ky){
	       $searchlist=db_factory::query("select words from ".TABLEPRE."witkey_hotwords where words like '%$ky%' and auto!=1 order by sort asc limit 5");
	    }else{
	        $searchlist=db_factory::query("select words from ".TABLEPRE."witkey_hotwords where auto!=1 order by count desc limit 5");
	    }
	    if($searchlist){
