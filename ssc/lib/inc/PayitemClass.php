...
class PayitemClass {
  ...
  public static function getPayitemRecord($strType='task',$objId,$code){
	...
	public static function validPayitemCount($arrPayitemNum,$endDate,$MaxTopTime=null){
		if(TOOL === TRUE){
		$arrGetDate = getdate ();
		...
		foreach ($arrPayitemNum as $k =>$v){
			if($k == 'tasktop'){
				if($arrPayitemNum[$k] > 0){
					if($MaxTopTime){						
						if($MaxTopTime >= $endTime){
							$tips['errors']['text_day'] = '你的置顶时间已到上限,无需购买！';
							kekezu::show_msg($tips,NULL,NULL,NULL,'error');
						}else{
							$IntMaxTopTime = $arrPayitemNum[$k] * 86400+$MaxTopTime; 
							if($IntMaxTopTime>$endTime){
								$tips['errors']['text_day'] = '置顶时间不能超过交稿时间：'.$endDate;
								kekezu::show_msg($tips,NULL,NULL,NULL,'error');
							}
						}
					}else{
						...
						if($IntMaxTopTime>$endTime){
							$tips['errors']['text_'.$k] = '置顶时间不能超过交稿时间：'.$endDate;
							kekezu::show_msg($tips,NULL,NULL,NULL,'error');
						}
					}
				}
	...
	public static function validPayitemCosts($totalCosts){
		...
		if($totalCosts > 0&&$userYe < $totalCosts){
			$tips['errors']['text_goodstop'] = '当前的余额不足，不能购买该增值服务';
			kekezu::show_msg($tips,NULL,NULL,NULL,'error');
		}
		}
	}
