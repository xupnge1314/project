...
class keke_register_class {
	...
	function user_register($reg_username, $reg_pwd, $reg_email, $reg_code,  $check_code = true, $old_pwd = null, $reg_user_type, $reg_user_type_xf, $is_oauth=null) {
		...
		$this->_reg_user_type = $reg_user_type;//增加user_type
	...
	function save_userinfo($reg_username, $reg_email, $reg_uid = null, $user_type,$reg_user_type_xf) {
		...
		$this->_member_obj->setUser_type ( $user_type );//增加user_type
		...
		if ($reg_member_uid) {
            ...
            $this->_space_obj->setUser_type($reg_user_type_xf);//修改用户实名认证类型
