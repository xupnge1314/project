...
final class match_time_class extends time_base_class {
	...
	public function task_tb_timeout() {		
		$task_list = db_factory::query(sprintf("select * from %switkey_task where task_status=2 and sub_time<'%s' and model_id=12",TABLEPRE,time()));
	...
	function task_work_timeout(){
		$task_list = db_factory::query(sprintf("select * from %switkey_task where task_status =5 and end_time<'%s' and model_id=12",TABLEPRE,time()));
		if(is_array($task_list)){
			foreach($task_list as $v){
				$task_obj = new match_task_class($v);
