...
define( "IN_KEKE" ,true);
	...
	$res = db_factory::execute("CREATE TABLE IF NOT EXISTS `".TABLEPRE."witkey_task_match_work` (
		...
		$col_info = db_factory::query("show COLUMNS FROM ".TABLEPRE."witkey_task_match_work where Field='quote' ");
		$col_info = $col_info[0];
		if($col_info){
			if($col_info["Type"]!="decimal(10,2)"){
				db_factory::execute("alter  table ".TABLEPRE."witkey_task_match_work  change column  quote quote decimal(10,2) default 0.00");
			}
		}
		else{
			db_factory::execute("alter table ".TABLEPRE."witkey_task_match_work add quote decimal(10,2) null default 0.00 ");
		}


		$col_info = db_factory::query("show COLUMNS FROM ".TABLEPRE."witkey_task_match_work where Field='quote_desc' ");
		$col_info = $col_info[0];
		if($col_info){
			if($col_info["Type"]!="text"){
				db_factory::execute("alter  table ".TABLEPRE."witkey_task_match_work  change column  quote_desc quote_desc text");
			}
		}
		else{
			db_factory::execute("alter table ".TABLEPRE."witkey_task_match_work add quote_desc text null default null ");
		}


		$col_info = db_factory::query("show COLUMNS FROM ".TABLEPRE."witkey_task_match_work where Field='cycle' ");
		$col_info = $col_info[0];
		if($col_info){
			if($col_info["Type"]!="int(11)"){
				db_factory::execute("alter  table ".TABLEPRE."witkey_task_match_work  change column  cycle cycle int(11)");
			}
		}
		else{
			db_factory::execute("alter table ".TABLEPRE."witkey_task_match_work add cycle int(11) null default null ");
		}
