		...
		if($res){
			if (is_array ( $service_ids )) {
				foreach ($service_ids as $key => $val){
					...
					$feed_arr = array ("feed_username" => array ("content" => $service_info['username'], "url" => "index.php?do=seller&id=$service_info[uid]" ), "action" => array ("content" => '发布了服务', "url" => "" ), "event" => array ("content" => "$service_info[title]", "url" => "index.php?do=goods&id=$val", "cash" => "$service_info[price]",
