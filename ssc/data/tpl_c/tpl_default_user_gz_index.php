<?php keke_tpl_class::checkrefresh('tpl/default/user/gz_index', '1436590012' );?><!DOCTYPE HTML>
          ...
          <div class="user-info">
            ...
            <div class="top-right">
              <div class="notice">
                ...
                <a href="index.php?do=user&view=transaction&op=released&intTaskStatus=0">待付款订单</a>
                <span class="number"><?php echo $intWaitPay['0']['count'];?></span>
              </div>

              <div class="notice">
                <i class="fa fa-bookmark"></i>
                <a href="index.php?do=user&view=transaction&op=released&intTaskStatus=3">待选需求</a>
                <span class="number"><?php echo $intChoose['0']['count'];?></span>
              </div>

              <div class="notice">
                <i class="fa fa-cubes"></i>
                <a href="index.php?do=user&view=transaction&op=orders&strStatus=wait">待付款服务</a>
                <span class="number"><?php echo $intShopPay['0']['count'];?></span>
              </div>

              <div class="notice">
                <i class="fa fa-comment"></i>
                <a href="index.php?do=user&view=gz&op=mark&type=1">待评价</a>
                <span class="number"><?php echo $intMarkG;?></span>
              </div>
      ...
      <div class="user-type">
          ...
          <div class="type-body1">
            <ul class="count">
              ...
              <li class="item"><span class="num"><?php echo kekezu::favorableRate($gUserInfo['uid'],2) ?><sub>%</sub></span>好评率：</li>
              <li class="item"><span class="num"><?php echo $gUserInfo['pub_num'];?></span>发布需求数：</li>
              <li class="item"><span class="num"><sub></sub><?php  echo keke_curren_class::output(floatval($floatPubTask[0]['cash']),-1);  ?></span>支付任务款：</li>
              <li class="item"><span class="num"><?php echo $floatPayService['0']['count'];?></span>购买服务数：</li>
          ...
          <table class="table table-hover count1">
            ...
            <tbody>
             <?php if(is_array($arrFavorite)) { foreach($arrFavorite as $v) { ?>
              <tr>
                <td><a href="index.php?do=goods&id=<?php echo $v['obj_id'];?>" class="list-desc"><?php echo $v['obj_name']?></a></td>
                <td><span class="money"><?php $a=ServiceClass::getShopByObj_id($v['obj_id']);echo $a['price'] ?></span></td>
                <td><a href="index.php?do=seller&id=<?php $a=ServiceClass::getShopByObj_id($v['obj_id']);echo $a['uid'] ?>"><?php $a=ServiceClass::getShopByObj_id($v['obj_id']);echo $a['username'] ?></a></td>
              </tr>
  <?php } } ?>
            </tbody>
          </table>
      ...
      <div class="user-type">
          ...
          <div class="type-list">
            ...
            <tbody>
            <?php if(is_array($arrRec)) { foreach($arrRec as $k => $v) { ?>
              <tr>
                <td><?php if($v['url']!='#') { ?><a href="<?php echo $v['url'];?>" class="list-desc"><?php echo $v['objname'];?></a><?php } else { ?><?php echo $v['objname'];?><?php } ?></td>
                <td><span class="money"><?php echo $v['cash'];?></span></td>
                <td><a href="index.php?do=seller&id=<?php echo $v['wiki_uid'];?>"><?php echo $v['wiki_username'];?></a></td>
              </tr>
           <?php } } ?>
            </tbody>
          </table>
